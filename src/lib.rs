...
impl Language {
    ...
    fn title(self) -> &'static str {
        match self {
            Self::En => "Head of Engineering",
            Self::Ru => "Руководитель разработки (Head of Engineering)",
        }
    }

    fn subtitle(self) -> &'static str {
        match self {
            Self::En => "Engineering leadership • AI-native software delivery • C++/Rust systems modernization",
            Self::Ru => "Руководство разработкой • разработка с ИИ-агентами • модернизация систем на C++/Rust",
        }
    }
...
pub fn App() -> impl IntoView {
    ...
    let popstate_listener = window_event_listener(leptos::ev::popstate, move |_| {
        set_language.set(initial_language());
    });
                ...
                on:click=move |_| {
                    ...
                    if let Ok(history) = window().history() {
                        let _ = history.push_state_with_url(
                            ...
                            "",
                            Some(next_language.target_path()),
                        );
                    }
                }
            >
...
mod tests {
    ...
    #[test]
    fn language_target_paths_match_supported_routes() {
        assert_eq!(Language::En.target_path(), "/CV/");
        assert_eq!(Language::Ru.target_path(), "/CV/ru/");
    }
